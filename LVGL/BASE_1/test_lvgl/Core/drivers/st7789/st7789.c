...
void ST7789_DrawImage(uint16_t x, uint16_t y, uint16_t w, uint16_t h, const uint16_t *data)
{
	...
	for (i = 0; i < w; i++){
		for (j = 0; j < h; j++) {
			uint8_t datam[] = {data[val+j] >> 8, data[val+j] & 0xFF};
			ST7789_WriteData(datam, sizeof(datam));
		}
		val+=j;
	}
	ST7789_UnSelect();
...
{
	ST7789_WriteString(10, 100,"Hello EMRE!", Font_16x26, WHITE, BLACK);
	...
	ST7789_WriteString(10, 10, "Font Deneme", Font_16x26, GBLUE, WHITE);
	ST7789_WriteString(10, 50, "Hello EMRE!", Font_7x10, RED, WHITE);
	ST7789_WriteString(10, 75, "Hello EMRE!", Font_11x18, YELLOW, WHITE);
	ST7789_WriteString(10, 100,"Hello EMRE!", Font_16x26, MAGENTA, WHITE);
	HAL_Delay(1000);
	...
	ST7789_DrawImage(0, 0, 240, 240, (const uint16_t *)tek);
	HAL_Delay(3000);

	ST7789_DrawImage(0, 0, 240, 240, (const uint16_t *)adi1);
	HAL_Delay(3000);
